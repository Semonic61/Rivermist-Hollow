...
/datum/preferences
	...
	var/voice_selection = VOICEPACK_TYPE_DEF	//RMH EDIT: choose voicepack
...
//			dat += "<a href='?_src_=prefs;preference=toggle_random;random_type=[RANDOM_SPECIES]'>Always Random Species: [(randomise[RANDOM_SPECIES]) ? "Yes" : "No"]</A><br>"
			...
			// LETHALSTONE EDIT END
			dat += "<b>Voicepack Type</b>: <a href='?_src_=prefs;preference=voiceselection;task=input'>[voice_selection]</a><BR>"
...
//						age = max(min( round(text2num(new_age)), AGE_MAX),AGE_MIN)
				...
				if ("voiceselection")
					...
					if(voicepack_type_input)
						if(voicepack_type_input == VOICEPACK_TYPE_DEF)
							voice_selection = VOICEPACK_TYPE_DEF
							to_chat(user, "<font color='red'>You will use your default species' voicepack.</font>")
						else if(voicepack_type_input == VOICEPACK_TYPE_MALE)
							generate_selectable_voicepacks()
							var voicepack_sel_input = input(user, "Choose your character's voicepack", "Voicepack") as null|anything in GLOB.selectable_voicepacks_male
							if(voicepack_sel_input)
								voice_selection = voicepack_sel_input
								to_chat(user, "<font color='red'>Your character will now use the '[lowertext(voicepack_sel_input)]' voicepack.</font>")
						else if(voicepack_type_input == VOICEPACK_TYPE_FEMALE)
							generate_selectable_voicepacks()
							var voicepack_sel_input = input(user, "Choose your character's voicepack", "Voicepack") as null|anything in GLOB.selectable_voicepacks_female
							if(voicepack_sel_input)
								voice_selection = voicepack_sel_input
								to_chat(user, "<font color='red'>Your character will now use the '[lowertext(voicepack_sel_input)]' voicepack.</font>")
...
/datum/preferences/proc/copy_to(mob/living/carbon/human/character, icon_updates = 1, roundstart_checks = TRUE, character_setup = FALSE, antagonist = FALSE)
	...
	//RMH Edit
	if(voice_selection == VOICEPACK_TYPE_DEF)
		character.voice_selection = null
	else
		generate_selectable_voicepacks()
		character.voice_selection = GLOB.selectable_voicepacks[voice_selection]
